...
//    m_Name = "Vu"; // compilation error
//    Student a("a", 123, "b", 1111);
...
QString Student::toString() const {
    QString retval;
    QTextStream os(&retval);                /* Write to the stream and return its string. */
    os << "[" << getClassName() << "]"
         << " name: " << m_Name
         << "; Id: " << m_StudentId
         << "; Year: " << yearStr()
...
//start id="undergrad"
Undergrad::Undergrad(QString name, long id, QString major, int year, int sat) : Student(name, id, major, year), m_SAT(sat)  /* The base class object is treated as a subobject of the derived object.
...
//start id="grad"
GradStudent::GradStudent(QString nm, long id, QString major, int yr, Support support) : Student(nm, id, major, yr), m_Support(support) { }
